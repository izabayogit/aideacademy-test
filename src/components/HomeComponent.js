...
export const HomeComponent = () => {
        ...
        const replyComment = (replyData) => {
                const replyObject = {
                        ...
                        image: "/assets/images/image-juliusomo.png",
                ...
                <DeleteProvider>
                        <div className=" main-container max-w-full ml-5 mr-5 md:max-w-[50%] lg:max-w-[50%] md:ml-auto md:mr-auto lg:ml-auto lg:mr-auto mt-10 mb-10">
                                {comments?.map((data) => (<div key={data.id}>
                                        ...
                                        {data.id === replyId ? (<div>
                                                <ReplyForm comment={data} replyComment={replyComment} commentData={data} myName={name} />
                                        </div>) : null}
                                        <div className="md:ml-5 lg:ml-10 border-l-[1px] borger-solid border-[#E5E4E2] mt-5" >
                                                {data.children.length ? (data.children.map((child) => (
                                                        <div className="ml-4 md:ml-5 lg:ml-10" key={child.id}>
                                                                {child.id != editId ? (<MainComment comment={child} handleReply={handleReply} upVote={upVote} downVote={downVote} myName={name} handleEdit={handleEdit} />) : <EditForm isMine={isMine} comment={child} updateComment={updateComment} myName={name} />}
                                ...
                                <Form updateUi={updateUi} myName={name} />
